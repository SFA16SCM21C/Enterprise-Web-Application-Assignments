...
//assign4
	public void insertprod(String prodtype, String id, String retailer, String name, String cond, Double price1, int quant, String sale, String rebate) throws IOException
			{
				SAXParserFactory saxParserFactory = SAXParserFactory.newInstance();
   				SAXParser saxParser = null;
			try 
		...
		newproduct.setQuant(quant);
		newproduct.setSale(sale);
		newproduct.setRebate(rebate);	
		 MyHandler handler = new MyHandler();
        try {
			saxParser.parse(new File("/usr/local/apache-tomcat-9.0.0.M26/webapps/a1/WEB-INF/ProductCatalog.xml"), handler);
		} catch (SAXException e) {
			e.printStackTrace();
		}
